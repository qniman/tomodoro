<x-emails.layout
    title="Подтвердите новый email — Tomodoro"
    preheader="Код подтверждения нового email: {{ $code }}. Действителен 30 минут."
>
    <x-slot:header>
    ...
    <p class="email-body-text" style="margin:0 0 8px;font-size:15px;line-height:1.7;color:#1d1d1a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
        {{ $name }}, вы запросили смену email в Tomodoro.
    </p>
    <p class="email-body-text" style="margin:0 0 28px;font-size:15px;line-height:1.7;color:#1d1d1a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
        Введите код ниже в настройках профиля, чтобы подтвердить <strong>{{ $newEmail }}</strong> как новый адрес:
    </p>
            ...
            <td align="center">
                <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        ...
                        @if($i === 3)<td style="width:16px;"></td>@endif
                        <td style="padding:0 4px;">
                            <div style="width:52px;height:64px;background:#f5f4f0;border:2px solid #e8e6dd;border-radius:12px;display:table-cell;vertical-align:middle;text-align:center;font-size:32px;font-weight:700;color:#1d1d1a;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;line-height:64px;">{{ $digit }}</div>
                        </td>
                        @endforeach
                    </tr>
                </table>
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom:32px;">
        <tr>
            <td align="center">
                <a href="{{ config('app.url') }}/app/settings?tab=profile"
                   target="_blank"
                   style="display:inline-block;background:#e5533a;color:#ffffff;font-size:15px;font-weight:600;text-decoration:none;padding:14px 36px;border-radius:10px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
                    Открыть настройки →
                </a>
            </td>
        </tr>
        ...
        <tr>
            <td style="border-top:1px solid #eeece4;padding-top:24px;" class="email-divider">
                <p style="margin:0;font-size:12px;line-height:1.7;color:#9a988d;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;" class="email-muted">
                    🛡 Если вы не запрашивали смену email — просто проигнорируйте это письмо. Ваш текущий адрес останется прежним.
                </p>
            </td>
        </tr>
    </table>
